...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/facebook/account", async (req, res) => {
    try {
      const { accessToken, adAccountId } = fetchAccountRequestSchema.parse(
        req.body,
      );
  ...
  app.post("/api/facebook/update-spend-cap", async (req, res) => {
    try {
      const { accessToken, adAccountId, spendCap } =
        updateSpendCapRequestSchema.parse(req.body);

      // Debug logging
      console.log("Received spend cap update request:", {
        adAccountId,
        spendCap,
        type: typeof spendCap,
      });
  ...
  app.post("/api/facebook/inactive-accounts", async (req, res) => {
    try {
      const {
        ...
        page = 1,
        limit = 20,
      } = inactiveAccountsRequestSchema.parse(req.body);
      ...
      const filteredAccounts = accountsData.data.filter((account: any) => {
        ...
        if (
          ...
          console.log(
            `[INACTIVE-ACCOUNTS] Filtering out account ${account.name} - zero/null spend_cap: ${account.spend_cap}`,
          );
          return false;
        }
        ...
        if (
          ...
          console.log(
            `[INACTIVE-ACCOUNTS] Filtering out account ${account.name} - spend_cap not above 1: original="${account.spend_cap}" parsed=${spendCap} currency=${account.currency}`,
          );
          return false;
        }

        console.log(
          `[INACTIVE-ACCOUNTS] Including account ${account.name} - spend_cap above 1: original="${account.spend_cap}" parsed=${spendCap} currency=${account.currency}`,
        );
      ...
      for (const account of filteredAccounts) {
        try {
          ...
          if (
            ...
            lastMonthSpend =
              parseFloat(insightsData.data[0].spend || "0") * 100; // Convert to cents
          }
          ...
          console.warn(
            `Failed to get insights for account ${account.id}:`,
            error,
          );
  ...
  app.post("/api/facebook/set-spend-cap-to-one", async (req, res) => {
    try {
      const { accessToken, adAccountId } = resetSpendCapRequestSchema.parse(
        req.body,
      );
  ...
  app.post("/api/facebook/business-manager-accounts", async (req, res) => {
    try {
      const {
        ...
        includeSpend = false,
        search,
      } = businessManagerAccountsRequestSchema.parse(req.body);

      console.log(`[BM-ACCOUNTS] Business Manager ID: ${businessId}`);
      console.log(
        `[BM-ACCOUNTS] ${after ? `Using cursor pagination with after: ${after.substring(0, 20)}...` : "Fetching first page (no cursor)"}`,
      );
      if (includeSpend) {
      ...
      console.log(`[BM-ACCOUNTS] Facebook API URL: ${url.replace(accessToken, 'TOKEN_HIDDEN')}`);
      console.log(`[BM-ACCOUNTS] Filters:`, JSON.stringify(filters));
      ...
      if (!response.ok) {
        const apiResponse: ApiResponse = {
          success: false,
          error:
            data.error?.message || "Failed to fetch Business Manager accounts",
        };
        return res.status(400).json(apiResponse);
      }

      console.log(
        `[BM-ACCOUNTS] Retrieved ${data.data?.length || 0} active accounts from Facebook API`,
      );
      console.log(
        `[BM-ACCOUNTS] Facebook paging info:`,
      ...
      if (data.data?.length > 0) {
        console.log(
          `[BM-ACCOUNTS-DEBUG] All ${data.data.length} account names:`,
          data.data.map((acc: any) => `"${acc.name}" (ID: ${acc.id})`),
        );
      }

      let accounts: InactiveAccount[] = (data.data || []).map(
        (account: any) => ({
          id: account.id,
          name: account.name,
          spend_cap: account.spend_cap,
      ...
      if (includeSpend && accounts.length > 0) {
        console.log(
          `[BM-ACCOUNTS] Fetching spend data for ${accounts.length} accounts...`,
        );

        console.log(
          "[BM-ACCOUNTS] Skipping spend data - will be loaded individually from frontend",
        );
      }
